    ...
    input_schema: {
      ...
      properties: {
        notation: {
          type: 'string',
          description:
            'Dice notation (e.g., "2d6", "1d20+5", "4d6kh3" for keep highest 3, "2d20kl1" for keep lowest/disadvantage)',
        },
        label: {
          type: 'string',
...
function rollDice(notation: string, label?: string): DiceResult | { error: string } {
  try {
    ...
    if (!match) {
      return {
        error: `Invalid dice notation: "${notation}". Use format like "2d6", "1d20+5", "4d6kh3"`,
      };
    }
