...
describe.skip('agent', () => {
  describe('runAgent', () => {
    ...
    it('should respect maxTurns limit', async () => {
      ...
      for await (const msg of runAgent({
        prompt:
          'Search for posts about AI, then search for posts about Vue, then search for posts about TypeScript',
        maxTurns: 2,
      })) {
        messages.push(msg);
      }
  ...
  describe('runAgentSync', () => {
    ...
    it('should handle tool use and return final result', async () => {
      ...
      const hasTimeToolUse = messages.some(
        (m) =>
          m.type === 'assistant' &&
          m.message?.content?.some(
            (c: { type: string; name?: string }) =>
  ...
  describe('tool integration', () => {
    it('should use getWeather tool for weather queries', async () => {
      ...
      const hasWeatherToolUse = messages.some(
        (m) =>
          m.type === 'assistant' &&
          m.message?.content?.some(
            (c: { type: string; name?: string }) =>
    ...
    it('should use rollDice tool for dice rolling', async () => {
      ...
      const hasDiceToolUse = messages.some(
        (m) =>
          m.type === 'assistant' &&
          m.message?.content?.some(
            (c: { type: string; name?: string }) =>
    ...
    it('should use getAuthorInfo for author queries', async () => {
      ...
      const hasAuthorToolUse = messages.some(
        (m) =>
          m.type === 'assistant' &&
          m.message?.content?.some(
            (c: { type: string; name?: string }) =>
