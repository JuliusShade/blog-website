...
Before Matt's video, I was thinking about this wrong. I'd start an interactive Claude Code session, add tasks to a state file, then try to run a loop _inside_ the conversation. Complex prompt engineering. Hooks to detect exit conditions. State management within the session.
...
The [commit that changed everything on 2026-01-11](https://github.com/JuliusShade/towles-tool/commit/ee6db37): `feat(ralph): auto-resume sessions per-task to prevent token burn`. That commit message tells the story. After the NEVER_AGAIN incident, I had to figure out how to preserve context between iterations but not fill up the context, find that sweet spot where Claude has enough context to be effective without burning through tokens unnecessarily.

::callout{type="warning"}
**Update (Jan 19):** The section below was a mistake. I was optimizing for the wrong thing - making loops _look_ fast instead of _work_ effectively. See [Goodhart's Law Ate My Context Window](/blog/goodharts-law-ate-my-context-window).
::
...
My first real test: [reviewing every blog post in my repository](https://github.com/JuliusShade/blog/pull/162). I created 37 tasks - one per post - ran ralph overnight, and woke up to grammar fixes, improved clarity, consistent formatting, all committed individually with meaningful messages.
...
- **towles-tool**: [github.com/JuliusShade/towles-tool](https://github.com/JuliusShade/towles-tool)
...
```bash
claude plugin marketplace add JuliusShade/towles-tool
...
_Thanks to [Matt Pocock](https://www.aihero.dev/tips-for-ai-coding-with-ralph-wiggum) for writing about Ralph Wiggum loops and inspiring this implementation._
